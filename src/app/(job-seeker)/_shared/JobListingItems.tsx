...
function JobListingListItems({ jobListing, organization }: {
  jobListing: Pick<
  ...
  return (
    ...
    >
          ...
          <div className="flex flex-col gap-1">
            ...
            {jobListing.postedAt != null && (
              <div className="text-sm font-medium text-primary @min-md:hidden">
                <Suspense fallback={jobListing.postedAt.toLocaleString()}>
      ...
      <CardContent className="flex flex-wrap gap-2">
        <JobListingBadges
          jobListing={jobListing}
          className={jobListing.isFeatured
            ? 'border-primary/35'
            : undefined}
        />
      </CardContent>
...
async function getJobListings(
  ...
  if (searchParams.type) {
    whereConditions.push(
      eq(JobListingTable.type, searchParams.type)
    );
  }
