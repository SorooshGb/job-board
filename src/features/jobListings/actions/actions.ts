...
export async function createJobListing(
  ...
  if (
    orgId == null || !(await hasOrgUserPermission('org:job_listings:create'))
  ) {
    return {
      error: true,
...
export async function updateJobListing(
  ...
  if (
    orgId == null || !(await hasOrgUserPermission('org:job_listings:update'))
  ) {
    return {
      error: true,
...
export async function toggleJobListingStatus(id: string) {
  ...
  await updateJobListingDb(id, {
    ...
    postedAt: newStatus === 'published' && jobListing.postedAt == null
      ? new Date()
      : undefined,
  });
