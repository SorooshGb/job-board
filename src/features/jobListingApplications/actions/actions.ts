...
export async function createJobListingApplication(
  ...
  const { success, data } = newJobListingApplicationSchema.safeParse(
    unsafeData
  );
...
export async function updateJobListingApplicationStage(
  ...
  const { success, data: stage } = z.enum(applicationStages).safeParse(
    unsafeStage
  );
  ...
  if (
    orgId == null || jobListing == null || orgId !== jobListing.organizationId
  ) {
    return {
      error: true,
      message: 'You don\'t have permission to update the stage',
...
export async function updateJobListingApplicationRating(
  ...
  const { success, data: rating } = z.number().min(1).max(5).nullable()
    .safeParse(
      unsafeRating
    );
