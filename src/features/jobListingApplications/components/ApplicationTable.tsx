    ...
    <div className="flex items-center gap-2">
      {table.getColumn('stage') && (
        <DataTableFacetedFilter
          ...
          title="Stage"
          options={applicationStages.toSorted(sortApplicationsByStage).map(
            stage => ({
...
function StageCell(
  { stage, jobListingId, userId, canUpdate }: {
...
function RatingCell(
  { rating, jobListingId, userId, canUpdate }: {
    rating: number | null;
    jobListingId: string;
    userId: string;
    canUpdate: boolean;
...
function ActionCell(
  { coverLetterMarkdown, resumeMarkdown, resumeUrl, userName }: {
    coverLetterMarkdown: ReactNode | null;
